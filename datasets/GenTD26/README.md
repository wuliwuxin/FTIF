...
所有数据文件都可以通过 container_ip 字段进行关联，该字段是容器 IP 地址的 MD5 哈希值。这确保了相同的原始 IP 地址对应相同的哈希值。服务性能指标（例如，模型加载延迟）可以通过此字段与系统性能指标（例如，GPU 利用率）进行关联。
...
如果您使用此数据集来分析扩散模型的请求特征、GPU 利用率模式或队列行为，请引用我们的 SoCC 论文：
```bibtex
...
如果您使用此数据集来分析不同时间段的请求分布模式变化或不同服务器上的 GPU 分解情况，请引用我们的 EuroSys 论文：
